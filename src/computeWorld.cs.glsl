...
void main()
{
	...
	if( gl_GlobalInvocationID.x < boneCount )
	{
		...
		uint rowHeadIndex = tableDepth * boneIndex;

		int parentIndex = HierarchyTable_array[rowHeadIndex];

		Bone rootBone = LocalBone_array[parentIndex];

		rootBone.t *= vec4(0,0,0,1);

		for( int i = 1; i < tableDepth; i++ )
		{
			int parentIndex = HierarchyTable_array[rowHeadIndex+i];
